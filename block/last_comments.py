        ...
        "last_comments" : [
            {
                ...
                "count_new"     : db.query(func.count(Comment)).filter(Comment.content_item == comment.content_item, Comment.created_at > request.user.last_activity).scalar() if request.user else 0,
                "new_index"     : db.query(func.count(Comment)).filter(Comment.content_item == comment.content_item, Comment.created_at <= request.user.last_activity).scalar() if request.user else 0,
            }
