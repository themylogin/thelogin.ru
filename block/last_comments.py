        ...
        "last_comments" : [
            {
                ...
                "new_index"     : db.query(func.count(Comment)).filter(Comment.content_item == comment.content_item, Comment.created_at <= request.user.last_visit).scalar() if request.user else 0,
            }
